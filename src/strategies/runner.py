...
class StrategyRunner:
    ...
    TelegramController is REQUIRED by your app, but it is created/wired in main.py.
    The runner just receives the ready-made controller and uses it.
    """
    def __init__(self, kite: Optional[KiteConnect] = None, telegram_controller: Any = None) -> None:
        self.log = logging.getLogger(self.__class__.__name__)
        self.kite = kite

        # Keep Telegram mandatory, but DO NOT construct it here (your main does that).
        ...
        self.executor = OrderExecutor(kite=self.kite, data_source=None)  # signature compatible with your repo
    ...
    def process_tick(self, tick: Optional[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            if hasattr(self.executor, "place_order"):
                ...
                # Original ZIP API (ENTRY + set GTT exits)
    ...
    def set_live_mode(self, val: bool) -> None:
        try: setattr(settings, "enable_live_trading", bool(val))
