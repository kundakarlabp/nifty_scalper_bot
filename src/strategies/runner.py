...
class StrategyRunner:
    ...
    Designed to be driven by a background loop (start()) and via Telegram commands.
    """
    ...
    def __init__(self, kite: Optional[KiteConnect] = None, telegram_controller: Any = None) -> None:
        ...
        self._tick_interval = max(2, int(getattr(settings.system, "position_sync_interval", 10)))
    ...
    def start(self) -> None:
        ...
        if self._loop_thread and self._loop_thread.is_alive():
            return
        self._stop.clear()
        self._loop_thread = threading.Thread(target=self._run_loop, name="runner-loop", daemon=True)
    ...
    def _run_loop(self) -> None:
        while not self._stop.is_set():
            try:
                ...
                # keep OCO / fills tidy if your executor exposes it
                if hasattr(self.executor, "sync_and_enforce_oco"):
                    try:
                        self.executor.sync_and_enforce_oco()
                    except Exception:
                        pass
            except Exception as e:
                self.log.warning("Loop iteration error: %s", e)
    ...
    def _fetch_spot_ohlc(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            self._last_fetch_ts = time.time() if isinstance(df, pd.DataFrame) and not df.empty else self._last_fetch_ts
    ...
    def _build_diag_bundle(self) -> Dict[str, Any]:
        ...
        checks.append({
            "name": "Data feed",
            "ok": age_s < 120,
            "detail": "fresh" if age_s < 120 else f"age={int(age_s)}s token={token} tf={settings.data.timeframe} lookback={settings.data.lookback_minutes}m",
        })
    ...
    # ------------- live-mode wiring -------------
    def _build_kite_from_settings(self) -> Optional["KiteConnect"]:
        if KiteConnect is None:
            self.log.warning("kiteconnect package not available.")
            return None
        key = getattr(settings.zerodha, "api_key", "")
        token = getattr(settings.zerodha, "access_token", "")
        if not key or not token:
            self.log.warning("Zerodha credentials missing; cannot enable live mode.")
            return None
        try:
            ...
            self.log.warning("Failed to build KiteConnect: %s", e)
            return None
    ...
    def set_live_mode(self, val: bool) -> None:
        ...
        if val:
            new_kite = self._build_kite_from_settings()
            if new_kite is None:
                self.log.warning("Requested live mode but kite=None; staying effectively in paper.")
                # ensure state consistent (paper)
                self.kite = None
                self.executor.kite = None
                if self.data_source is not None and hasattr(self.data_source, "kite"):
                    self.data_source.kite = None
                return

            # wire everywhere
            self.kite = new_kite
            self.executor.kite = new_kite
            if self.data_source is not None and hasattr(self.data_source, "kite"):
                self.data_source.kite = new_kite
                try:
                    self.data_source.connect()
                except Exception:
                    pass
