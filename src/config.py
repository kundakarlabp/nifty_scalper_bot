#src/config.py
"""
Professional Trading Bot Configuration (finalized)
- Flat, environment-driven settings with strict validation
- Live-equity sizing controls (use broker equity with safety floor + refresh cadence)
- Production-friendly defaults for Nifty options scalping
"""
...
class TradingSettings(BaseSettings):
    ...
    # === RISK (with live equity controls) ===
    risk_use_live_equity: bool = Field(default=True, env="RISK_USE_LIVE_EQUITY")
    risk_default_equity: float = Field(default=30000.0, env="RISK_DEFAULT_EQUITY")  # fallback
    ...
    risk_risk_per_trade: float = Field(default=0.01, env="RISK_RISK_PER_TRADE")  # 1%
    ...
    risk_max_daily_drawdown_pct: float = Field(default=0.04, env="RISK_MAX_DAILY_DRAWDOWN_PCT")  # 4%
    risk_max_position_size_pct: float = Field(default=0.10, env="RISK_MAX_POSITION_SIZE_PCT")  # 10%
    ...
    # === DATA / BACKTEST ===
    data_source: str = Field(default="kiteconnect", env="DATA_SOURCE")
    data_cache_enabled: bool = Field(default=True, env="DATA_CACHE_ENABLED")
    data_cache_ttl_seconds: int = Field(default=60, env="DATA_CACHE_TTL_SECONDS")
...
class DevelopmentSettings(TradingSettings):
    ...
    risk_risk_per_trade: float = 0.01
...
# Global settings instance (import-time load)
...
def validate_critical_settings() -> None:
    ...
    - Telegram creds required only if TELEGRAM_ENABLED=true.
    - Core logical constraints.
    """
    errors = []

    # Broker credentials only if live
    if settings.enable_live_trading:
        if not settings.zerodha_api_key:
            errors.append("ZERODHA_API_KEY is required for live trading")
        if not settings.zerodha_api_secret:
            errors.append("ZERODHA_API_SECRET is required for live trading")
        if not settings.zerodha_access_token:
            errors.append("ZERODHA_ACCESS_TOKEN is required for live trading")
    ...
    if settings.instruments_max_lots < settings.instruments_min_lots:
        errors.append("INSTRUMENTS_MAX_LOTS must be >= INSTRUMENTS_MIN_LOTS")

    # Strategy relationships
    if settings.strategy_ema_fast >= settings.strategy_ema_slow:
        errors.append("STRATEGY_EMA_FAST must be < STRATEGY_EMA_SLOW")

    if settings.strategy_atr_tp_multiplier <= settings.strategy_atr_sl_multiplier:
        errors.append("STRATEGY_ATR_TP_MULTIPLIER should be > STRATEGY_ATR_SL_MULTIPLIER")

    # Risk sanity
    if settings.risk_use_live_equity and settings.risk_min_equity_floor <= 0:
        errors.append("RISK_MIN_EQUITY_FLOOR must be > 0 when RISK_USE_LIVE_EQUITY=true")
