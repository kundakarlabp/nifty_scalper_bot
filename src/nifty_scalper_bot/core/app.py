...
async def _stop_polling_fallback_safely(polling_fallback: Any, *, reason: str) -> None:
    try:
...
async def _polling_failover_supervisor_iteration(
    ...
    *,
    ...
    if not bool(market_open):
        await _stop_polling_fallback_safely(polling_fallback, reason="market_closed")
    ...
    if not decision.activate:
        ...
        if now_mono - recovered_since >= max(0.0, float(recover_cooldown or 0.0)):
...
# Re-exported here so existing callers (runner, tests) that import these names
# from nifty_scalper_bot.core.app continue to work unchanged.
from nifty_scalper_bot.core.history_readiness import (  # noqa: E402
    HistoryPolicyDecision,
    HistoryReadiness,
    RuntimeHistoryResult,
    SelectedOptionHistoryReadiness,
    _bar_timestamp,
    _count_bars_from_provider,
    _count_symbol_bars,
    _get_bars_from_provider,
    _get_cached_quote,
    _hydration_status_map,
    _pick_atm_option_symbols_from_basket,
    _status_for_role,
    build_symbol_hydration_status,
    compute_history_readiness,
    compute_selected_option_history_readiness,
