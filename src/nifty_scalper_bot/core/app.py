...
def sync_symbol_history_to_runner(
    ...
    if mdm is not None:
        try:
            mdm_before = len(mdm.get_ohlc_bars(symbol) or [])
        except Exception:
            mdm_before = 0
    ...
    LOGGER.info(
        "SELECTED_OPTION_HYDRATION_SYNC_RESULT symbol=%s mdm_before=%s runner_before=%s runner_after=%s required=%s ready=%s",
        symbol,
        mdm_before,
        runner_before,
        runner_after,
    ...
    *,
    ...
    if runner_bars < required and mdm_bars >= required:
        sync_result = ctx.strategy_runner.sync_history_from_mdm(
            ...
            reason=source,
            role=resolve_symbol_history_role(ctx, symbol),
            request_if_short=False,
        )
        runner_bars = int(getattr(sync_result, "indicator_bars", runner_bars) or runner_bars)
...
def initialize_components(settings: Settings | None = None) -> BotContext:
    ...
    async def runtime_history_ensurer(
        symbol: str,
        *,
        role: str,
        phase: str,
    ...
    if callable(getattr(strategy_runner, "set_runtime_history_ensurer", None)):
        strategy_runner.set_runtime_history_ensurer(runtime_history_ensurer)
    else:
        LOGGER.error(
            "CANONICAL_HISTORY_ENSURER_INJECTION_FAILED reason=setter_missing",
            extra={"event": "CANONICAL_HISTORY_ENSURER_INJECTION_FAILED", "reason": "setter_missing"},
        )
...
async def ensure_symbol_runtime_history(
    ...
    *,
    ...
    effective_required = max(1, int(required_bars) if required_bars is not None else int(policy.required_bars))
    effective_target = max(
        effective_required,
        int(target_bars) if target_bars is not None else int(policy.target_bars),
    )
    policy = replace(policy, required_bars=effective_required, target_bars=effective_target)
    ...
    if policy.allow_broker_fetch:
        hydration = await mdm.ensure_history(
            ...
            required_bars=policy.required_bars,
            target_bars=policy.target_bars,
            days=_history_lookback_days(policy.target_bars),
            role=role,
            phase=phase,
            reason=reason,
            minimum_only=False,
        ...
        if getattr(hydration, "failure_reason", None):
            failure_reason = str(hydration.failure_reason)
    else:
        try:
            mdm_bars_skip = len(mdm.get_ohlc_bars(symbol) or [])
        except Exception:
            mdm_bars_skip = 0
        if mdm_bars_skip < policy.required_bars:
            failure_reason = "broker_fetch_not_allowed"
    sync_result: Any | None = None
    if runner is None or not callable(getattr(runner, "sync_history_from_mdm", None)):
        failure_reason = failure_reason or "runner_sync_history_missing"
    else:
        ...
        if getattr(sync_result, "failure_reason", None):
            failure_reason = failure_reason or str(sync_result.failure_reason)
    try:
    ...
    readiness = compute_history_readiness(symbol=symbol, role=role, required_bars=policy.required_bars, mdm_bars=mdm_bars, runner_bars=runner_bars, indicator_bars=indicator_bars)
    minimum_ready = bool(readiness.minimum_ready and failure_reason is None)
    ...
    *,
    ...
    if callable(reconcile_fn):
        ...
        for sym in [s for s in committed.get("symbols", []) if s]:
            try:
                tok = int(_im.get_token(str(sym))) if _im else 0
                if tok > 0:
        ...
        try:
            ...
            async def _fetch_symbol(symbol: str) -> tuple[str, list[Any]]:
                ...
                runtime = await ensure_symbol_runtime_history(
                    ...
                    target_bars=hydration_max_bars,
                )
