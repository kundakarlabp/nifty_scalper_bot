...
This module closes two runtime gaps without creating a new history owner:
- completed OHLC retention is independent of raw-tick retention;
- option strategies receive canonical active spot/futures symbol identities.

MDM remains completed-history SSOT, DataHub remains a facade, strategies read the
shared IndicatorEngine, and OrderManager is deliberately not involved.
"""
...
def ensure_mdm_ohlc_capacity(manager: Any) -> int:
    """Upgrade an existing MDM completed-history capacity without touching ticks."""
    ...
    if isinstance(ohlc, MutableMapping):
        for symbol in list(ohlc.keys()):
            try:
                ohlc[symbol] = _resize_deque(ohlc[symbol], capacity)
            except Exception:  # noqa: BLE001 - retain existing history if adaptation fails
                continue
        if isinstance(ohlc, defaultdict):
...
def install_mdm_ohlc_capacity_contract() -> bool:
    """Decouple MarketDataManager OHLC capacity from its raw-tick cache."""
    try:
    ...
    def __init__(self: Any, *args: Any, **kwargs: Any) -> None:
        ...
        if existing is not None:
            replacement: defaultdict[str, deque[Any]] = defaultdict(
                lambda: deque(maxlen=capacity)
            )
            try:
                for symbol, rows in dict(existing).items():
                    replacement[str(symbol)] = _resize_deque(rows, capacity)
                self._ohlc = replacement
            except Exception:  # noqa: BLE001 - original state remains usable
                pass
    ...
    def get_ohlc_bars(
        ...
        try:
            ...
            rows = list(original_get(self, symbol) or [])
...
def _basket_from_data_hub(data_hub: Any | None) -> Mapping[str, Any]:
    ...
    if callable(getter):
        try:
            basket = getter()
        except Exception:  # noqa: BLE001 - strategy context stays fail-closed
            basket = None
...
def resolve_active_underlying_symbols(
    ...
    raw_spot = context.get("spot_symbol") or basket.get("spot_symbol") or "NSE:NIFTY"
    raw_future = (
...
# StrategyManager imports strategy_context_builder before the production MDM
# instance is normally constructed. Existing MDM instances are still upgraded
# lazily by get_ohlc_bars/history_capacity_for.
