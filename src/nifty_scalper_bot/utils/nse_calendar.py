"""NSE equity-derivatives trading holiday calendar.

The static calendar is sourced from NSE F&O circular NSE/FAOP/71777 dated
12 December 2025.  Runtime overrides permit emergency exchange notices without
requiring a code deployment:

- ``NSE_ADDITIONAL_HOLIDAYS=YYYY-MM-DD,YYYY-MM-DD``
- ``NSE_REMOVED_HOLIDAYS=YYYY-MM-DD,YYYY-MM-DD``

Weekend handling remains in :mod:`nifty_scalper_bot.utils.market_hours`.
"""
...
def _runtime_overrides(additional_raw: str, removed_raw: str) -> tuple[frozenset[date], frozenset[date]]:
...
def calendar_available(year: int) -> bool:
    """Return whether a built-in calendar exists for ``year``."""
