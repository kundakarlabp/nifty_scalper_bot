...
class _BurstDedupFilter(logging.Filter):
    ...
    def __init__(self, window_sec: float) -> None:
        """Initialise the filter with a deduplication window.

        Args:
            window_sec: Duration in seconds to suppress duplicate records.
    ...
    def filter(self, record: logging.LogRecord) -> bool:  # type: ignore[override]
        """Return ``True`` when the record is not a recent duplicate.

        Args:
            record: Candidate logging record for evaluation.
        ...
        try:
            ...
            LOGGER.error(
                ...
                exc,
                extra={
                    "event": "logging_burst_dedup_error",
                    "bypass_filters": True,
                },
                exc_info=exc,
            )
...
class _RateLimitFilter(logging.Filter):
    ...
    def __init__(self, interval_sec: float) -> None:
        """Initialise the filter with a rate limit interval.

        Args:
            interval_sec: Minimum seconds between identical records.
    ...
    def filter(self, record: logging.LogRecord) -> bool:  # type: ignore[override]
        """Return ``True`` when the record is outside the rate limit window.
...
def setup_logging(level: str = "INFO") -> None:
    ...
    try:
        ...
        handler.addFilter(
            _EventSamplingFilter(
                os.getenv(
                    "LOG_SAMPLE_EVENT_REGEX",
                    r"^regime_multiplier_applied$",
                ),
                _resolve_int("LOG_SAMPLE_EVERY", 50),
            )
        )
...
def _apply_noisy_overrides() -> None:
    ...
    try:
        ...
        LOGGER.error(
            ...
            exc,
            extra={
                "event": "logging_noisy_overrides_error",
                "bypass_filters": True,
            },
            exc_info=exc,
        )
...
def _install_filters_once() -> None:
    ...
    try:
        ...
        LOGGER.error(
            ...
            exc,
            extra={
                "event": "logging_install_filters_error",
                "bypass_filters": True,
            },
            exc_info=exc,
        )
...
class _DedupFilter(logging.Filter):
    ...
    def __init__(self, window_sec: float) -> None:
        """Initialise the filter for the provided window duration."""
...
class _EventSamplingFilter(logging.Filter):
    ...
    def __init__(self, pattern: str | None, every: int) -> None:
        """Initialise the filter with sampling configuration."""
...
def _resolve_int(name: str, default: int) -> int:
    ...
    Args:
        name: Environment variable name.
        default: Fallback integer value when parsing fails.
