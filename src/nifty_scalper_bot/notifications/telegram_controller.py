...
class TelegramBot:
    ...
    async def _start_polling_if_needed(self) -> None:
        """Ensure PTB polling is running when webhook transport is disabled."""
        ...
        async def _runner(application: Application) -> None:
        ...
        self._polling_task = loop.create_task(_runner(app), name="telegram-polling")
    ...
    async def cmd_resolve(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if resolver is not None:
            try:
                metadata = resolver.lookup(symbol)
            except Exception as exc:  # noqa: BLE001
                log.error(
                    "Failure in cmd_resolve: %s",
                    exc,
                    extra={
                        "event": "cmd_resolve_lookup_failed",
                        "symbol": symbol,
                        "error": str(exc),
                    },
                )
        lines: list[str] = [f"symbol={symbol}"]
        if metadata:
            exchange = str(metadata.get("exchange") or "n/a").upper()
            tradingsymbol = str(metadata.get("tradingsymbol") or "n/a").upper()
            token = metadata.get("instrument_token")
            lines.append(f"resolver.exchange={exchange}")
            lines.append(f"resolver.tradingsymbol={tradingsymbol}")
            lines.append(f"resolver.token={token}")
        else:
            lines.append("resolver=unavailable")
        candidates: list[t.Any] = []
        cache_token: t.Any | None = None
        if mdm_raw is not None:
            mdm = t.cast(t.Any, mdm_raw)
            with suppress(Exception):
                mapping = getattr(mdm, "_token_by_symbol", {})
                if isinstance(mapping, dict):
                    cache_token = mapping.get(symbol)
            try:
                ...
                log.error(
                    ...
                    exc,
                    extra={
                        "event": "cmd_resolve_candidate_failed",
                        "symbol": symbol,
                        "error": str(exc),
                    },
                )
        if cache_token is not None:
            lines.append(f"mdm.cache_token={cache_token}")
        if candidates:
    ...
    async def cmd_margin(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            available = float(getattr(decision, "available", 0.0) or 0.0)
            remaining = available - est_needed
            status_lines = [
                f"💰 Margin for {symbol} BUY x{quantity}",
                f"Required: ₹{est_needed:.2f}",
                f"Available: ₹{available:.2f} ({available_source})",
                f"Post-trade: ₹{remaining:.2f}",
                f"Order type: {decision.order_type} x{decision.quantity}",
            ]
            if decision.ok:
    ...
    @command_meta(
        ...
        "Dry-run freshness, gate, and margin checks for an order.",
    )
    async def cmd_plan(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        data_hub = getattr(self.deps, "data_hub", None)
        fresh_ok = False
        fresh_meta: dict[str, t.Any] = {}
        fresh_reason = "-"
        if data_hub is None:
            fresh_reason = "data_hub_unavailable"
        else:
            try:
                fresh_ok, raw_meta = data_hub.is_fresh(symbol)
            except Exception as exc:  # noqa: BLE001
                fresh_ok = False
                fresh_reason = f"error:{self._short_reason(str(exc))}"
            else:
                if isinstance(raw_meta, t.Mapping):
                    fresh_meta = dict(raw_meta)
                fresh_reason = str(fresh_meta.get("reason") or "-")

        def _fmt_ms(value: t.Any) -> str:
            if isinstance(value, (int, float)):
                return f"{float(value):.0f}"
            return "-"

        eff_ms = _fmt_ms(fresh_meta.get("effective_ms"))
        threshold_ms = _fmt_ms(fresh_meta.get("threshold_ms"))
        mono_ms = _fmt_ms(fresh_meta.get("mono_age_ms"))
        ...
        if risk_manager is None:
            gate_ok = False
            gate_codes = ["risk_manager_unavailable"]
        else:
            try:
                gate_ok, raw_reasons = risk_manager.risk_gate_should_trade()
            except Exception as exc:  # noqa: BLE001
                gate_ok = False
                gate_codes = [f"error:{self._short_reason(str(exc))}"]
            else:
                codes = {canonical(reason) for reason in raw_reasons if reason}
                gate_codes = sorted(codes) if codes else []

        regime_manager = getattr(self.deps, "regime_manager", None)
        regime_ok = True
        regime_reasons: tuple[str, ...] = tuple()
        regime_snapshot: RegimeSnapshot | None = None
        if regime_manager is not None:
            try:
                regime_ok = regime_manager.can_trade(
                    context={
                        "component": "telegram_plan",
                        "symbol": symbol,
                        "side": side,
                        "quantity": quantity,
                    }
                )
                regime_reasons = regime_manager.get_filter_reasons()
                regime_snapshot = regime_manager.get_latest_snapshot()
            except Exception as exc:  # noqa: BLE001
                regime_ok = False
                regime_reasons = (f"error:{self._short_reason(str(exc))}",)
        ...
        if order_manager is None:
            margin_ok = False
            margin_reason = "order_manager_unavailable"
        else:
            planner = getattr(order_manager, "_pre_trade_decision", None)
            if not callable(planner):
                margin_ok = False
                margin_reason = "margin_planner_unavailable"
            else:
                try:
                    decision, context = planner(  # type: ignore[misc]
                        symbol=symbol,
                    ...
                    margin_ok = False
                    margin_reason = f"error:{self._short_reason(str(exc))}"
                else:
                    margin_ok = bool(decision.ok)
                    margin_reason = decision.reason or ""

        needed = decision.est_required if decision else None
        available = decision.available if decision else None
        available_source = (
            context.get("available_source", "unknown") if decision else "unknown"
        )
        planned_qty = decision.quantity if decision else quantity
        planned_type = decision.order_type if decision else "-"

        def _fmt_float(value: t.Any) -> str:
            if isinstance(value, (int, float)):
                return f"{float(value):.2f}"
            return "n/a"

        gate_reason_text = ",".join(gate_codes) if gate_codes else "-"
        regime_reason_text = ",".join(regime_reasons) if regime_reasons else "-"
        margin_reason_text = margin_reason or "-"
        outcome_ok = fresh_ok and gate_ok and margin_ok and regime_ok
        outcome = "ok" if outcome_ok else "blocked"

        regime_line = "regime_gate=unavailable"
        if regime_manager is not None:
            snapshot_details = "-"
            if regime_snapshot is not None:
                adjustments = {}
                if isinstance(regime_snapshot.adjustments, t.Mapping):
                    adjustments = regime_snapshot.adjustments
                blocklist = adjustments.get("blocklist") or adjustments.get("block")
                if isinstance(blocklist, (list, tuple, set)):
                    block_display = ",".join(str(item) for item in blocklist)
                else:
                    block_display = str(blocklist or "-")
                sizing = adjustments.get("sizing_multiplier")
                base_label = (
                    f"{regime_snapshot.regime} ({regime_snapshot.confidence:.2f})"
                )
                if isinstance(sizing, (int, float)):
                    snapshot_details = f"{base_label} size×{float(sizing):.2f}"
                else:
                    snapshot_details = base_label
                snapshot_details += f" block={block_display}"
            regime_line = (
                f"regime_gate={regime_ok} reasons={regime_reason_text}"
                f" snapshot={snapshot_details}"
            )

        margin_line = (
            "margin="
            f"{margin_ok} reason={margin_reason_text} "
            f"type={planned_type} qty={planned_qty} "
            f"est={_fmt_float(needed)} avail={_fmt_float(available)} "
            f"avail_src={available_source}"
        )
        summary_lines = [
            f"/plan {symbol} {side} {quantity}",
            (
                "fresh="
                f"{fresh_ok} eff_ms={eff_ms}/{threshold_ms} "
                f"mono_ms={mono_ms} reason={fresh_reason}"
            ),
            f"risk_gate={gate_ok} reasons={gate_reason_text}",
            regime_line,
            margin_line,
            f"outcome={outcome}",
        ]
    ...
    async def cmd_flow(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if risk_manager is not None:
            balance = self._coerce_float_value(
                getattr(risk_manager, "current_balance", None),
                field="risk.balance",
            )
        ...
        broker_has_balance = bool(broker) and balance is not None and balance > 0
        balance_source = "broker" if broker_has_balance else "fallback"
