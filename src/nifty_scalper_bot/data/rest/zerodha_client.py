...
class ZerodhaKiteClient(BaseBrokerClient):
    ...
    def _normalize_margin_payload(
        ...
        available_value = self._coerce_positive_float(target.get("available"))
        if isinstance(target.get("available"), Mapping):
            available_mapping = cast(Mapping[str, Any], target.get("available"))
            ...
            nested_available = self._coerce_positive_float(target.get("cash"))
    ...
    def load_instruments(self, exchange: str = _DEFAULT_EXCHANGE) -> list[dict]:
        ...
        response = self._make_request(
            "GET", f"/instruments/{exchange}", raw_response=True
        )
