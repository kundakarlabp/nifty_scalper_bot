...
* CandleBuilder  – time-bucket (1-min replace) per symbol; emits ONLY closed candles.
...
class CandleBuilder:
    ...
    * Bucket boundary = replace(second=0, microsecond=0, nanosecond=0).
    ...
    * OHLC invariant validated before emission; violating candles are dropped.
    ...
    def _process(self, tick: ValidatedTick) -> Optional[Candle]:
        sym = tick.symbol
        ts = tick.timestamp
        # Performance optimization: ~14x faster than .floor('1min')
