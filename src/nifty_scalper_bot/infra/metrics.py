    ...
    def record_heartbeat_flush(
        ...
        *,
        ...
        if latency_seconds is not None and latency_seconds >= 0.0:
            ...
            try:
                self._heartbeat_flush_latency.observe(latency_value)
            except Exception as exc:  # noqa: BLE001
                LOGGER.error(
                    "Failure in record_heartbeat_flush latency: %s",
                    exc,
                    extra={
                        "event": "metrics_heartbeat_flush_latency_error",
                        "outcome": outcome,
                    },
                )
        with self._lock:
            self._heartbeat_flush_events.append(
                (reference_time, outcome, float(latency_value or 0.0))
            )
            self._purge_expired(reference_time)
            if success:
                self._last_successful_flush = reference_time
            last_success = self._last_successful_flush
        if last_success is not None:
            age_value = max(0.0, reference_time - last_success)
            try:
                self._heartbeat_flush_age.set(age_value)
            except Exception as exc:  # noqa: BLE001
                LOGGER.error(
                    "Failure in record_heartbeat_flush age gauge: %s",
                    exc,
                    extra={
                        "event": "metrics_heartbeat_flush_age_error",
                        "outcome": outcome,
                    },
                )
        LOGGER.info(
            "Condition met: persistent_flush_recorded",
            extra={
