"""Broker-order classification and deterministic reconciliation compatibility patch.

PositionManager owns durable ledger/quarantine serialization.  This overlay only
classifies broker orders, maintains the in-memory registries under the native
state lock, and exposes the outer entry blocker/reconciliation compatibility API.
"""
...
def _status(row: Any) -> str:
    raw = str(
        _get(row, "status", "order_status", "state") or "UNKNOWN"
    ).strip().upper()
    normalized = getattr(
...
def apply_patches() -> None:
    ...
    def add_pending_order(
        ...
        if order is not None:
            ...
            row = _ledger_row(
                existing=(getattr(self, "_broker_order_ledger", {}) or {}).get(oid),
    ...
    def reconcile_broker_orders(
        ...
        for raw in rows:
            ...
            with self._lock:
                ledger_row = dict(
                    (getattr(self, "_broker_order_ledger", {}) or {}).get(oid, {})
                )
    ...
    def current_entry_protection_blocker(
        ...
        with self._lock:
            ledger = dict(getattr(self, "_broker_order_ledger", {}) or {})
