...
class RuntimeBracketManager(LedgerBracketManager):
    ...
    def _retry_orphan_ledger_block(
        ...
        if symbol:
            try:
                ...
                _core.LOGGER.error(
                    "FILL_LEDGER_ORPHAN_POSITION_CHECK_FAILED "
                    "bracket_id=%s symbol=%s error=%s",
                    bracket_id,
                    symbol,
                    exc,
                    extra={
        ...
        if flat is not True:
            _core.LOGGER.warning(
                "FILL_LEDGER_ORPHAN_BLOCK_RETAINED "
                "bracket_id=%s symbol=%s broker_flat=%s",
                bracket_id,
                symbol or "unknown",
                flat,
                extra={
    ...
    def confirm_partial_entry_fill(
        ...
        with suppress(Exception):
            self.save_state()
        _core.LOGGER.critical(
            "PARTIAL_ENTRY_PROTECTED order_id=%s symbol=%s "
            "planned_qty=%s filled_qty=%s fill_price=%.2f",
            order_id,
            bracket.symbol,
            planned_quantity,
            quantity,
    ...
    def reconcile_filled_exit_order(
        ...
        with suppress(Exception):
            self.save_state()
        _core.LOGGER.info(
            "EXIT_ORDER_CORRELATED bracket_id=%s order_id=%s "
            "symbol=%s source=%s reason=%s qty=%s fill_price=%s",
            bracket.bracket_id,
            order_id,
            symbol,
            source,
    ...
    def _close_bracket(
        ...
        *,
        ...
        _core.LOGGER.info(
            "BRACKET_CLOSED bracket_id=%s symbol=%s "
            "close_source=%s side=%s qty=%s entry=%s exit=%s pnl=%s",
            bracket.bracket_id,
            bracket.symbol,
            close_source,
            bracket.side,
