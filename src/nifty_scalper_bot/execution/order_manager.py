...
class OrderManager:
    ...
    def _pre_trade_decision(
        ...
        *,
        ...
        try:
            metrics.reference_price_source_total.labels(source=meta.source).inc()
    ...
    def _build_order_payload(
        ...
        *,
        ...
        # Only pass a trading symbol if it is explicitly provided or already
        # formatted as a derivative identifier. Leaving it empty allows the
        # broker client to resolve NFO symbols like FUT/CE/PE contracts.
        if ":" in symbol:
            _exchange, maybe_symbol = symbol.split(":", maxsplit=1)
            if maybe_symbol:
                payload["tradingsymbol"] = maybe_symbol
        else:
        ...
        if env_exch:
            ...
            exchange, _tradingsymbol = symbol.split(":", maxsplit=1)
