...
def login_page(e: str = "") -> HTMLResponse:
    ...
    <input id="password_input" type=password name=password autofocus placeholder="Password" aria-label="Password">
...
def dashboard(request: Request) -> HTMLResponse:
    ...
    for label, key, sec in FIELDS:
        ...
        rows += f'<label for="input_{key}">{label} <span class=badge>{key}</span></label><input id="input_{key}" name="{key}" value="{shown}" placeholder="(unchanged)">'
    ...
    <label for="req_token">Request Token (recommended)</label><input id="req_token" name="request_token" placeholder="paste request_token from login URL">
    <label for="acc_token">…or Access Token (if you already have it)</label><input id="acc_token" name="access_token" placeholder="paste access_token directly">
