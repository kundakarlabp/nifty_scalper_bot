...
def compute(
    *,
    ...
    tick_lag = (
        float("inf")
        if tick_ts is None
        else max(0.0, (now_utc - tick_ts).total_seconds())
    )
    bar_lag = (
        None if bar_close is None else max(0.0, (now_utc - bar_close).total_seconds())
    )
    tick_ok = tick_ts is not None and tick_lag <= max_tick_lag_s
    bar_ok = bar_lag is None or bar_lag <= max_bar_lag_s
    ok = tick_ok and bar_ok
