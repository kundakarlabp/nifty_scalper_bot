...
# Install security (apt-transport-https, ca-certificates) and build dependencies
# FIX: Packages are listed correctly with line continuation for perfect shell parsing
...
# Copy application code and run build-time checks
COPY . /app
RUN python scripts/verify_runtime.py

# Download instruments CSV (with retry logic for robustness)
