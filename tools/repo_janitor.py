...
def find_orphans(files: List[Path]) -> List[str]:
    ...
    roots = set(
        f
        for f in py_files
        if any(f.endswith(ep) for ep in ENTRY_POINTS) or f.startswith("tests/")
    )
    reachable = set(roots)
    ...
    orphans = [
        f for f in py_files if f not in reachable and not f.endswith("__init__.py")
    ]
...
def main():
    ...
    Path("JANITOR_REPORT.json").write_text(
