...
async def test_cmd_plan_reports_margin_block() -> None:
        ...
        def _pre_trade_decision(
            ...
            *,
            ...
            decision = MarginDecision(
                ok=False,
                reason="MARGIN needed=150.00 available=100.00 buffer=0.90",
                order_type="NRML",
                quantity=0,
                est_required=150.0,
                available=100.0,
    ...
    class _StubHub:
        def is_fresh(self, symbol: str) -> tuple[bool, dict[str, float | None]]:
            return True, {
                "effective_ms": 120.0,
                "threshold_ms": 500.0,
                "mono_age_ms": 80.0,
    ...
    class _StubRegimeManager:
        ...
        def get_latest_snapshot(self) -> RegimeSnapshot:  # noqa: D401 - test stub
            return RegimeSnapshot(
                ...
                adjustments={"sizing_multiplier": 0.9, "blocklist": ["trend"]},
            )

    risk_manager = types.SimpleNamespace(risk_gate_should_trade=lambda: (True, ()))
    order_manager = _StubOrderManager()
    deps = TelegramDeps(
        token="x",
        chat_id=1,
        ...
        regime_manager=_StubRegimeManager(),
    )
    bot = TelegramBot(deps)
    chat = _DummyChat(1)
    update = types.SimpleNamespace(effective_chat=chat)
    ...
    message = chat.sent[-1]
    assert "/plan NIFTY BUY 2" in message
    assert "fresh=True" in message
    assert "risk_gate=True" in message
    assert "regime_gate=True" in message
    assert "margin=False" in message
