...
def test_fill_validation_rejects_non_economic_values() -> None:
...
def test_identical_fill_replay_is_idempotent_and_conflict_is_rejected(tmp_path) -> None:
    ...
    assert store.record_fill(original) is True
    assert store.record_fill(original) is False
    ...
    with pytest.raises(FillConflictError):
        store.record_fill(changed)


def test_scaled_long_exits_use_quantity_weighted_fill_pnl() -> None:
...
def test_partial_exit_reports_realized_component_without_claiming_closure() -> None:
    pnl = calculate_realized_pnl(
        [
            _leg(
                fill_id="entry",
...
def test_short_position_pnl_uses_reverse_price_direction() -> None:
    pnl = calculate_realized_pnl(
        [
            _leg(
                fill_id="short-entry",
...
def test_over_exit_and_wrong_exit_side_fail_closed() -> None:
    entry = _leg(
        fill_id="entry",
        order_id="entry-order",
        kind="ENTRY",
...
def test_sqlite_reload_preserves_order_and_economics(tmp_path) -> None:
    path = tmp_path / "fills.db"
    first = BracketFillLedgerStore(path)
    legs = [
        _leg(
            fill_id="entry",
            order_id="entry-order",
    ...
    for leg in legs:
        assert first.record_fill(leg) is True
