...
@pytest.mark.asyncio
async def test_explicit_target_bars_override_policy_default_to_mdm() -> None:
    mdm = MDM(); runner = Runner()
    ...
    assert result.required_bars == 30
    assert result.target_bars == 75
    assert mdm.calls[-1][1]["required_bars"] == 30
    assert mdm.calls[-1][1]["target_bars"] == 75


@pytest.mark.asyncio
async def test_runner_callback_contract_uses_real_orchestrator_target_bars() -> None:
    from nifty_scalper_bot.strategies.runner import StrategyRunner

    mdm = MDM(); runner = Runner()
    runtime_ctx = ctx(mdm, runner)
    scheduling_runner = StrategyRunner.__new__(StrategyRunner)
    scheduling_runner._logger = SimpleNamespace(info=lambda *a, **k: None, warning=lambda *a, **k: None, error=lambda *a, **k: None)
    scheduling_runner._normalize_symbol = lambda s: str(s)
    scheduling_runner._runtime_history_ensure_inflight = {}
    scheduling_runner._runtime_history_ensure_roles = {}

    async def runtime_history_ensurer(symbol: str, *, role: str, phase: str, reason: str, required_bars: int, target_bars: int | None = None):
        return await app.ensure_symbol_runtime_history(
            runtime_ctx,
            symbol,
            role=role,
            phase=phase,
    ...
    scheduling_runner.set_runtime_history_ensurer(runtime_history_ensurer)
    assert scheduling_runner._schedule_runtime_history_ensure(
        "NFO:CE",
        role="selected_option",
        phase="startup",
    ...
    assert mdm.calls[-1][1]["target_bars"] == 75
    assert mdm.calls[-1][1]["role"] == "selected_option"
