...
@pytest.mark.asyncio
async def test_deferred_retry_stops_when_hydration_statuses_ready(
    monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture
) -> None:
    calls: list[str] = []
    ...
    async def _build_basket(*args, **kwargs):
        calls.append("build")
        return {"selected_ce": "NFO:NIFTY26JUN23700CE"}
    ...
    await app._deferred_basket_hydration_retry(
        ctx, configured_mode="LIVE", max_attempts=3, delay_seconds=0
    )

    assert calls == ["build", "start", "recompute"]
    assert ctx.data_ready is True
    assert ctx.data_pipeline_ready is True
    assert ctx.startup_phase == "data_ready"
    assert "DEFERRED_BASKET_RETRY_SUCCESS" in caplog.text
    assert "DEFERRED_BASKET_HYDRATION_FAILED" not in caplog.text
    assert "hydration_status_by_symbol" not in caplog.text
