...
class BacktestRunner:
    ...
    def tick(self):
        ...
        # First, manage exits on any open trade
        if self.active_trade:
            self._check_exit(current_bar)

        # Then, if no active trade and risk is okay, evaluate entries
    ...
    def _check_exit(self, current_bar: pd.Series) -> None:
        ...
        exit_price = None
        # We may have attached these attrs on entry (dynamic attributes are allowed)
        sl = getattr(trade, "stop_loss", None)
        tp = getattr(trade, "target", None)
        ...
        if exit_price is not None:
            ...
            logger.info(f"{current_bar.name} - Exit trade at {exit_px:.2f}")
    ...
    def _check_entry(self, current_dt: datetime, current_bar: pd.Series) -> None:
        ...
        logger.info(f"Signal received at {current_dt}: {sig}")
        ...
        setattr(trade, "target", sig["take_profit"])

        self.session.add_trade(trade)
        self.active_trade = trade
        logger.info(
            f"{current_dt} - New Trade: {trade.direction} {trade.quantity} @ {trade.entry_price:.2f} "
    ...
    def generate_report(self) -> None:
        """Generates a detailed performance report using quantstats (if available)."""
        ...
        # This is a coarse approximation (trade-to-trade), not bar-level CAGR.
        net_pnls = [t.net_pnl for t in self.session.trade_history]
        ...
        try:
            ...
            # Fallback to simple text + CSV
            self._generate_simple_report()

    def _generate_simple_report(self) -> None:
        """A basic text-based report as a fallback, plus CSV of trade history."""
        print("\n" + "=" * 50)
