...
def test_tick_shows_l1(monkeypatch) -> None:
    _prep(monkeypatch)
    tc = TelegramController(status_provider=lambda: {}, l1_provider=lambda: {"bid": 1})
    sent: list[str] = []
...
def test_force_eval_runs_runner(monkeypatch) -> None:
    _prep(monkeypatch)
    ...
    tc = TelegramController(status_provider=lambda: {}, runner_tick=fake_tick)
    sent: list[str] = []
    tc._send = lambda text, parse_mode=None: sent.append(text)
