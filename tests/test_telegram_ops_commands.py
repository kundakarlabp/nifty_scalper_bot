...
def test_debug_windows_and_diagnostics(monkeypatch) -> None:
    ...
    assert subs_lines[0].startswith("subs_count=")
    assert "111 FULL" in subs_lines[1]

    start_idx = len(sent)
    assert tc._handle_cmd("/diag", "") is True
    diag_lines = sent[start_idx:]
    assert diag_lines[0].startswith("ws_connected=")
    assert any(line.startswith("per_token_age_ms") for line in diag_lines)
    assert any(line.startswith("stale_counts") for line in diag_lines)
