...
def create_test_dataframe(
    length: int = 100, *, trending_up: bool = True, constant_price: bool = False
) -> pd.DataFrame:
    ...
    if constant_price:
        ...
        prices = (
            np.linspace(100.0, 120.0, length)
            if trending_up
            else np.linspace(120.0, 100.0, length)
        )
...
def test_generate_signal_returns_valid_structure():
    ...
    old_min_score = settings.strategy.min_signal_score
    old_conf = settings.strategy.confidence_threshold
    settings.strategy.min_signal_score = 1
    settings.strategy.confidence_threshold = 1
    try:
        ...
        if sig:
            ...
            assert isinstance(sig.get("confidence"), float)
            assert sig.get("entry_price") > 0
            assert sig.get("target") != sig.get("stop_loss")
    finally:
        settings.strategy.min_signal_score = old_min_score
        settings.strategy.confidence_threshold = old_conf
...
def test_signal_direction_on_trends():
    ...
    try:
        ...
        assert sig_down.get("action") == "SELL"
    finally:
        settings.strategy.min_signal_score = old_min_score
        settings.strategy.confidence_threshold = old_conf
