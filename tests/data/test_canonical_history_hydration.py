...
def _mdm(stored: list[dict] | None = None) -> MarketDataManager:
    ...
    mdm._canonical_symbol = lambda s: (
        "NSE:NIFTY" if str(s) in {"NIFTY", "NSE:NIFTY"} else str(s)
    )
    ...
    def ingest(_symbol, rows):
        ...
        for row in rows:
            if (
                ...
                or not {"open", "high", "low", "close"}.issubset(row)
            ):
                continue
...
async def test_cache_sufficient_for_target_skips_broker() -> None:
    ...
    result = await mdm.ensure_history(
        "NSE:NIFTY", required_bars=30, target_bars=300, reason="t"
    )
...
async def test_minimum_only_skips_when_minimum_met() -> None:
    ...
    mdm.fetch_history = fetch
    result = await mdm.ensure_history(
        "NSE:NIFTY", required_bars=30, target_bars=300, reason="t", minimum_only=True
    )
...
async def test_larger_inflight_satisfies_smaller_request() -> None:
    ...
    big = asyncio.create_task(
        mdm.ensure_history("NSE:NIFTY", required_bars=30, target_bars=300, reason="big")
    )
    await asyncio.sleep(0)
    small = await mdm.ensure_history(
        "NSE:NIFTY", required_bars=30, target_bars=30, reason="small"
    )
    await big
    assert calls == 1
...
@pytest.mark.asyncio
async def test_smaller_inflight_then_larger_request_fetches_second_call() -> None:
    mdm = _mdm([])
    calls: list[int] = []
    ...
    await asyncio.sleep(0)
    large = await mdm.ensure_history(
        "NSE:NIFTY", required_bars=30, target_bars=300, reason="large"
    )
    await small
    assert len(calls) == 2
...
async def test_failed_request_cleans_inflight_and_can_retry() -> None:
    ...
    mdm.fetch_history = fetch
    failed = await mdm.ensure_history(
        "NSE:NIFTY", required_bars=30, target_bars=30, reason="fail"
    )
    assert failed.failure_reason
    assert mdm._history_inflight == {}
    retry = await mdm.ensure_history(
        "NSE:NIFTY", required_bars=30, target_bars=30, reason="retry"
    )
...
async def test_joined_only_caller_reports_not_started() -> None:
    ...
    first = asyncio.ensure_future(
        mdm.ensure_history(
            "NSE:NIFTY", required_bars=30, target_bars=300, reason="first"
        )
    )
    ...
    second = asyncio.ensure_future(
        mdm.ensure_history(
            "NSE:NIFTY", required_bars=30, target_bars=300, reason="joiner"
        )
    )
...
def test_historical_bar_imports_through_candle_engine_projection_not_raw_ticks() -> (
    None
):
    mdm = _storage_mdm()
    bar = {
