...
def test_identical_basket_recommit_preserves_active_future_live_readiness() -> None:
    ...
    # Production commits the basket and then DataHub's basket subscription
    # hardening requests each symbol explicitly through MDM.
    mdm.set_active_contract_basket(basket)
    assert mdm.request_token_subscription(token, symbol=fut)
    ...
    # Model the periodic SSOT re-commit plus DataHub's repeated subscription
    # request. Both operations must be idempotent for an unchanged basket/token.
    mdm.set_active_contract_basket(basket)
    assert mdm.request_token_subscription(token, symbol=fut)
