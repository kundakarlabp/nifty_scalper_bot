...
def test_runner_reuses_mirrored_engine_without_reentering_mdm_registry() -> None:
    ...
    assert first is second
    assert calls == 1
    assert runner._mirror_authoritative_candle_engine.__func__.__module__ == (
        "nifty_scalper_bot.strategies.runner"
    )


def test_candle_cache_has_no_import_time_method_replacement() -> None:
    from pathlib import Path

    source = (
        Path(__file__).resolve().parents[2] / "src/nifty_scalper_bot/core/__init__.py"
    ).read_text()
    assert "_install_runner_candle_engine_cache_patch" not in source
