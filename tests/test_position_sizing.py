...
def test_lots_from_premium_cap(monkeypatch):
    ...
    monkeypatch.setattr(cfg, "EXPOSURE_CAP_PCT", 10.0, raising=False)
    lots, meta = lots_from_premium_cap(100, 25, cfg, 10_000)
    assert meta["unit_notional"] == 2_500
    assert meta["cap"] == 1_000
    assert lots == 0


def test_basic_sizing(monkeypatch):
    monkeypatch.setattr(cfg, "EXPOSURE_CAP_PCT", 100.0, raising=False)
    sizer = _sizer()
...
def test_min_lots_rescue_when_affordable(monkeypatch):
...
def test_allow_min_one_lot_when_cap_small(monkeypatch):
    monkeypatch.setenv("RISK__ALLOW_MIN_ONE_LOT", "1")
    monkeypatch.setattr(cfg, "EXPOSURE_CAP_PCT", 1.0, raising=False)
    sizer = _sizer()
    qty, lots, diag = sizer.size_from_signal(
        entry_price=200.0,
        stop_loss=180.0,
