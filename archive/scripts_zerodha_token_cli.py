...
Usage:
  python -m nifty_scalper_bot.scripts.zerodha_token_cli --write-env
"""
...
def main() -> None:
    ...
    ap.add_argument(
        "--write-env", action="store_true", help="Write KITE_ACCESS_TOKEN into .env"
    )
    ...
    api_secret = (
        os.getenv("KITE_API_SECRET") or getpass("KITE_API_SECRET (hidden): ").strip()
    )
    ...
    if args.open:
        try:
            ...
            LOGGER.exception(
                "Failed to open login URL in browser: %s",
                exc,
                extra={"event": "zerodha_token_browser_error"},
            )
    ...
    try:
        ...
        LOGGER.exception(
            "Failed to exchange request_token with Kite: %s",
            exc,
            extra={"event": "zerodha_token_exchange_failed"},
        )
        sys.exit("Failed to exchange request_token. See logs for details.")
