...
RULES = (
    ...
    ("market-data", ("/data/", "/instruments/", "instrument_manager.py"), ("tests/data", "tests/core", "tests/instruments")),
    ("execution", ("/execution/",), ("tests/execution", "tests/integration/test_canonical_bo_end_to_end.py", "tests/test_execution_path_contract.py")),
    ...
    ("deployment", ("deploy/", "ops/", "dockerfile", "railway.toml"), ("tests/core/test_release_guard.py", "tests/test_deployment_release_guard.py")),
    ("agent-tooling", ("agents.md", ".agents/", "copilot-instructions.md", "repo_map.md", "scripts/agent_"), ("tests/tools", "tests/architecture/test_agent_skills_catalog.py")),
)
...
def build(root: Path, files: Sequence[str]) -> Plan:
    normalized = tuple(dict.fromkeys(Path(item).as_posix().lstrip("./") for item in files if item.strip()))
    ...
    for area, markers, candidates in RULES:
        if any(any(marker in path for marker in markers) for path in lowered):
            areas.append(area)
            tests.extend(candidate for candidate in candidates if (root / candidate).exists())
    if any(path.startswith("src/") for path in normalized):
        tests.extend(candidate for candidate in ("tests/architecture/test_canonical_bo_ownership.py", "tests/test_execution_path_contract.py") if (root / candidate).exists())
    tests = list(dict.fromkeys(tests))
    commands = ["python -m compileall -q src dashboard"]
    commands.append("python -m pytest -q " + " ".join(tests) if tests else "python -m pytest -q tests/architecture")
    commands.append("python -m pytest -q")
    return Plan(normalized, tuple(areas or ["unclassified"]), tuple(tests), tuple(commands))
...
def markdown(plan: Plan) -> str:
    ...
    lines.extend(["", "## Areas", "", *(f"- `{item}`" for item in plan.areas), "", "## Focused tests", ""])
    ...
    lines.extend(["", "## Commands", "", "```bash", *plan.commands, "```", "", "> Focused checks accelerate feedback; the complete suite and final-head CI remain mandatory before squash merge.", ""])
    return "\n".join(lines)
